...
export type IdeaToPrdData = {
  projectName: string;
  projectType: string;
  slides: IdeaToPrdStep[];
};

function parseMarkdownFile(filePath: string, fallbackSummary: string[]): { title: string; summary: string[]; extra: Record<string, any> } {
  try {
...
export function extractIdeaToPrdData(projectRoot: string): IdeaToPrdData {
  const slides: IdeaToPrdStep[] = [];
  ...
  if (fs.existsSync(manifestPath)) {
    try {
      ...
      if (manifest.product) projectName = manifest.product;
    } catch (e) {}
  }
  ...
  if (!competitorPath) {
    try {
      ...
      if (fs.existsSync(docsDir)) {
        ...
        if (matches.length > 0) competitorPath = path.join(docsDir, matches[0]);
      }
    } catch (e) {}
  }
  ...
  if (!techPath) {
    try {
      ...
      if (fs.existsSync(openModsDir)) {
        ...
        if (matches.length > 0) techPath = path.join(openModsDir, matches[0]);
      }
    } catch (e) {}
  }
  ...
  if (!prdPath) {
    ...
    try {
      ...
      if (fs.existsSync(skillsDir)) {
        ...
        if (match) prdPath = path.join(skillsDir, match);
      }
    } catch(e) {}
  }
  ...
  if (!archPath) {
    try {
      ...
      if (fs.existsSync(skillsDir)) {
        ...
        if (match) archPath = path.join(skillsDir, match);
      }
    } catch(e) {}
  }
  ...
  if (!epicsPath) {
    try {
      ...
      if (fs.existsSync(skillsDir)) {
        ...
        if (match) epicsPath = path.join(skillsDir, match);
      }
    } catch(e) {}
  }
