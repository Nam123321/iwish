...
/** Persona participating in a pre-debate round. */
...
/** Result of an internal pre-debate round between agent personas. */
export interface PreDebateResult {
  resolved: boolean;
  recommendation: string;
  rationale: string;
  evidence: string[];
  debateLog: DebatePersona[];
  options: string[];
}

/** A single entry in the decisions log. */
...
/** Tracks the halt/resume state of individual sub-agents. */
export interface AgentHaltState {
  agentId: string;
  storyId: string;
  halted: boolean;
...
/** Aggregated decisions summary for an epic sprint. */
export interface DecisionsSummary {
  epicId: string;
  generatedAt: string;
  totalDecisions: number;
  autoResolvedCount: number;
  userEscalatedCount: number;
  timeoutAutoSelectCount: number;
  avgResolutionTimeMs: number;
  entries: DecisionLogEntry[];
}

/** Persisted state for the prompt queue (JSON backup). */
interface PromptQueueState {
  queue: PromptQueueItem[];
  agentStates: AgentHaltState[];
  lastUpdated: string;
}
...
let agentHaltStates: Map<string, AgentHaltState> = new Map();
 ...
 */
function getQueueStatePath(projectRoot: string): string {
 ...
 */
function getDecisionsLogPath(projectRoot: string, epicId: string): string {
  return path.join(projectRoot, I_WISH_OUTPUT_DIR, `decisions-log-epic-${epicId}.json`);
}

/**
 * Persist current in-memory queue state to a JSON file for crash recovery.
 */
async function persistQueueState(projectRoot: string): Promise<void> {
  const statePath = getQueueStatePath(projectRoot);
  const state: PromptQueueState = {
    queue: [...promptQueue],
 ...
 */
export async function restoreQueueState(projectRoot: string): Promise<void> {
  const statePath = getQueueStatePath(projectRoot);
  if (!fs.existsSync(statePath)) {
    return;
  }
  try {
    const state: PromptQueueState = await fs.readJson(statePath);
    promptQueue = state.queue || [];
    agentHaltStates = new Map(
      (state.agentStates || []).map((s) => [s.agentId, s]),
    );
 ...
 */
export function executePreDebate(
  question: string,
  storyContext: string,
  relevantDocs: Record<string, string>,
): PreDebateResult {
  ...
  for (const docName of docNames) {
    ...
    for (const line of lines) {
      ...
      const questionTokens = questionLower
        ...
        .split(/\s+/)
        .filter((t) => t.length > 3);
      const matchCount = questionTokens.filter((token) =>
  ...
  const devPersona: DebatePersona = {
    agentId: 'dev-agent',
  ...
  const architectPersona: DebatePersona = {
 ...
 */
export async function logAutoResolution(
  projectRoot: string,
  storyId: string,
  question: string,
  rationale: string,
 ...
 */
export async function submitToPromptQueue(
  projectRoot: string,
  question: string,
  options: string[],
  storyId: string,
...
// ---------------------------------------------------------------------------
// Task 4: displayNextPrompt + timeout handling
// ---------------------------------------------------------------------------

/** Result of displaying a prompt to the user. */
export interface DisplayPromptResult {
 ...
 */
export function displayNextPrompt(): DisplayPromptResult {
 ...
 */
export async function handlePromptTimeout(
  projectRoot: string,
): Promise<{ autoSelected: boolean; item: PromptQueueItem | null }> {
  if (promptQueue.length === 0) {
 ...
 */
export async function routeUserResponse(
  projectRoot: string,
  storyId: string,
  response: string,
): Promise<string | null> {
 ...
 */
export async function partialHalt(
  projectRoot: string,
  storyId: string,
  agentId?: string,
): Promise<void> {
 ...
 */
export function getAgentHaltStates(): AgentHaltState[] {
 ...
 */
export function isAgentHalted(agentId: string): boolean {
 ...
 */
export async function generateDecisionsSummary(
  projectRoot: string,
  epicId: string,
 ...
 */
export function getQueueSnapshot(): PromptQueueItem[] {
  return [...promptQueue];
}
 ...
 * @param projectRoot - Absolute path to the project root.
 */
export async function clearQueue(projectRoot: string): Promise<void> {
  promptQueue = [];
  agentHaltStates = new Map();
  queueIdCounter = 0;
  await persistQueueState(projectRoot);
 ...
 */
function findWaitingAgentId(storyId: string): string | null {
