...
- **Technical & Architectural Impact:** Does it alter the database schema (ERD), API contracts, or require new third-party integrations/libraries?
  - **MANDATORY LIGHT RISK SCAN:** You MUST load the Review Agent (`/.agent/agents/review-agent.md`) using the Edge Case Guardian skill (`/.agent/skills/Hit/SKILL.md`) to run a quick scan on **P4 (Data Integrity)**, **P5 (Integration Failure)**, and **P6 (Permission & Security)** to identify major architectural risks before finalizing the impact level.
...
**[P] Party Mode** - Call the King-Kai, Piccolo, and Vegeta Agents to debate the routing verdict.
