...
- **STITCH-TO-CODE PIPELINE ENFORCEMENT**: Tránh lỗi Schema-Driven Layout. Khi implement UI từ thiết kế Stitch, UI phải tuân thủ nghiêm ngặt cấu trúc cây DOM theo HTML/CSS đầu ra của Stitch (không dựa vào backend schema). Bắt buộc phải tải và tham chiếu HTML/CSS từ Stitch làm "Source of Truth" duy nhất cho giao diện.
- **VISUAL ENFORCEMENT GATE**: Bắt buộc có bước "Visual Validation" trước khi hoàn tất subtask. Phải đối chiếu thiết kế thực tế so với CSS/HTML contract từ Stitch (hoặc dùng `stitch-design-taste`).
- **MANDATORY UX GUARDIAN CHECK**: Đối với các tác vụ làm Frontend UI/UX, bắt buộc phải đọc `.agent/skills/ux-guardian/SKILL.md` và kiểm tra file `ux-patterns.yaml`. Nếu hành vi UI chưa có trong Master Record, hãy cắm cờ `[NEW_UX_PATTERN_PROPOSAL]`.
- **CRITICAL ANTI-HALLUCINATION**: Do not blindly guess or assume standard boilerplate paths for decorators, utilities, or shared components. Always use CodeGraphContext (CGC) MCP tools (e.g., `find_symbol`, `search_codebase`) or FeatureGraph to locate the exact file and export name before writing the import statement.
