...
## Phase 5: Log Instincts for Whis Evolution (MANDATORY)

> Mỗi UX inconsistency phát hiện trong Phase 3 PHẢI được ghi vào Machine Memory.

11. Với mỗi vi phạm UX Pattern tìm thấy, append 1 dòng JSONL vào `.agent/memory/instincts.jsonl`:
    ```jsonl
    {"ts":"YYYY-MM-DD","src":"audit-ux","ctx":"ui,<behavior-type>","bad":"<inconsistent-pattern>","good":"<master-pattern>","sev":<1-5>,"file":"<component-file>"}
    ```
12. Nếu FalkorDB khả dụng, tạo Node `(:Instinct)` và liên kết `[:AFFECTS]` tới Component/Feature tương ứng trên FeatureGraph.
