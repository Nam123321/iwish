...
## 📌 OVERVIEW
This workflow implements an automated security auditing, verification, and remediation cycle in an isolated sandbox. It orchestrates a multi-agent routing loop between `review-agent` (auditing), `Tien-Shinhan-agent` (exploit validation), and `Vegeta-agent` (patching).
...
### Phase 2: Sandbox Exploit Verification 🧪 (Agent: `Tien-Shinhan-agent`)
1. For each high/critical vulnerability candidate, the `Tien-Shinhan-agent` generates or loads a Proof-of-Concept (PoC) exploit script (e.g., `poc_exploit.py` or `test-exploit.js`).
...
### Phase 3: Automated Patching & Re-Verification 🩹 (Agent: `Vegeta-agent`)
1. **Patch Generation:** Route the confirmed vulnerability metadata and the PoC script to `Vegeta-agent`.
2. **Implementation:** `Vegeta-agent` designs and applies code fixes directly targeting the root cause.
