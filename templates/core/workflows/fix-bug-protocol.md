...
8c. **🤖 Page-Agent Bug Reproduction (nếu UI/State/Integration bug):**
    - Load SKILL: `@{project-root}/.agent/skills/page-agent-Tien-Shinhan/SKILL.md`
...
11. **Edge Case Guardian — 8-Pillar scan:**
    - Load SKILL: `@{project-root}/.agent/skills/Hit/SKILL.md`
...
19. **Code review & Tien-Shinhan Simulator Guardian Audit (Fat-Guardian Adversarial):**
    - Load SKILL: `@{project-root}/.agent/skills/Tien-Shinhan-simulator-guardian.md`
    - Tien-Shinhan-agent (hoặc Auditor Agent) phải kích hoạt Simulator để phân loại Code Fix (1 trong 13 Types) và chấm điểm.
    ...
    - **Delta Lock Loop Protection:** Check `fixAttempts` trong `_iwish-output/bug-tracker.yaml`.
      - Nếu trả về `[FIXABLE]` và Delta Score cải thiện `>= 0.5`, bẻ khóa workflow trở lại Phase 5 bắt Vegeta-agent fix tiếp.
...
26. **Cập nhật Bug Scorecard** trong `_iwish-output/bug-tracker.yaml`:
    ...
    - id: BUG-XXXX
      ...
      fixAttempts:
        - attempt: 1
          date: 2026-03-10
          rootCause: "Surface-level: Next.js Vegeta mode quirk"
...
| Data corruption risk | Mandatory Kira (Data Piccolo) review |
