...
1. **Agent Persona**: `${IWISH_HOME}/generated-agents/<agent-name>/<agent-name>.md` (XML-based persona with menu, following existing agent conventions like Piccolo-agent.md. The frontmatter MUST include: name, description, inputs: [], outputs: [], mcp_tools_required: [], subagent_triggers: [])
