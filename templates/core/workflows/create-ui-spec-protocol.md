...
**Goal:** Generate a per-story UI specification with component hierarchy, responsive layout, design tokens, and interaction patterns. User must approve the UI spec before the story proceeds to `Vegeta-agent-story`.

**This is the "Discovery Track" workflow** — it runs between `create-story` and `Vegeta-agent-story` for stories that have frontend UI components.
...
4. This mapping becomes the AUTHORITATIVE reference for Vegeta-agent-story coding
```

### Visual Diff Verification (Post-Coding)

After coding FE tasks in `Vegeta-agent-story`:

```
1. Run Vegeta-agent server (`npm run Vegeta-agent` or equivalent)
...
### Stitch-to-Vegeta-agent Handoff Amendment

After Stitch screens are approved by user, **APPEND** this section to the UI Spec:

```
## §{N}. Stitch-to-Vegeta-agent Handoff
> Auto-generated by /stitch-first-Vegeta-agent after Stitch approval ({date})
...
(These become validator items in Vegeta-agent-story checklist)
...
> [!IMPORTANT]
> This handoff section is the **SINGLE SOURCE OF TRUTH** for Vegeta-agent-story implementation.
> Vegeta-agent agent reads ONE file (UI Spec + Amendment) — no hunting across multiple docs.
> See `/stitch-first-Vegeta-agent` workflow for full scoring and variant logic.
