...
1. **Thematic Clustering**:
   ...
   - Group them into failure modes:
     - **Mechanical**: Syntax errors, broken imports, missing types (Upgrade logic in `ast-health.js` or `Tien-Shinhan-agent`).
     - **Contextual**: Agent missed an edge case in a specific domain (Upgrade `Edge Case Guardian` or `Kira Data Piccolo-agent`).
