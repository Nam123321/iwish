...
[P] Party Mode - get expert feedback before Vegeta-agent
...
Copy this prompt to start Vegeta-agent:

\`\`\`
quick-Vegeta-agent {finalFile}
\`\`\`

This ensures the Vegeta-agent agent has clean context focused solely on implementation.
```
...
- MUST update status to 'ready-for-Vegeta-agent'.
- MUST rename file to `tech-spec-{slug}.md`.
- MUST provide clear next-step guidance and recommend fresh context for Vegeta-agent.
