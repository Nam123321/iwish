...
### 0. Compiler Validation Gate

Before reviewing logic, verify the code actually compiles.
- Run `pnpm build`, `nest build`, or `tsc --noEmit`
- If compilation fails (e.g., `MODULE_NOT_FOUND`), immediately flag as a CRITICAL SEVERITY finding and HALT. Do not proceed to diff generation until the build is green.
...
```xml
<invoke-task>Review {diff_output} using {project-root}/_iwish/core/tasks/review-adversarial-general.xml</invoke-task>
```
