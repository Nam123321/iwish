...
**Persona:** orch-agent (Master Orchestrator) + Piccolo-agent (Piccolo)
...
| Layer 2 | `add_code_to_graph("<file>")` per file | **Vegeta-agent** (Vegeta Agent) | Cuối `/Vegeta-story` và `/fix-bug` |
| Layer 3 | `FALKORDB_PORT=<PORT> cgc index --force .` full re-index | **Master Roshi** (Trunks) | Đầu sprint (`/sprint-planning`) và cuối epic (`/retrospective`) |
| IGNORE_DIRS sync | `sync-ignore-dirs.sh --check` | **Master Roshi** (Trunks) | Đầu mỗi sprint |
...
| `/Vegeta-story` | Context-aware development + auto graph refresh (Layer 2) | Both |
