...
3. **Run Regression & Architectural Impact Check** (MANDATORY for `epic` or `plan` severity pivots):
   - If the pivot severity is classified as `epic` or `plan`, you MUST invoke the Review Agent (`/.agent/agents/review-agent.md`) loading the Edge Case Guardian SKILL (`/.agent/skills/Hit/SKILL.md`) to run a Regression & Architectural Impact Check on the proposed redirection.
