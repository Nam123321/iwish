...
- `predict-personas.md` provides the **5-Persona Debate Protocol** (Piccolo, Security, Performance, UX, Devil's Advocate).
...
| `technical` | Gate 2: `/Vegeta-agent-story` (Before Implementation Plan) | DB schema, API contracts, **Module Depth (Deletion Test)**, performance, security, and state management. |
...
- **Story-Level Auto-Resolution:** For Story-level gates (Gate 1: Business, Gate 2: Technical, Gate 3: Drift), before asking the user a question, you MUST simulate a 1-round `party-mode` debate internally with other relevant agent personas (e.g., Vegeta-agent, Piccolo-agent, Bulma-agent).
...
- **Protocol:** Execute the full 7-step Debate Protocol defined in `@.agent/fragments/predict-personas.md`:
  ...
  3. Each of the 5 personas (Piccolo, Security, Performance, UX, Devil's Advocate) analyzes independently
