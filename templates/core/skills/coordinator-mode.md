...
Bộ quy trình này áp dụng cho Orchestrator (`orch-agent`), `King-Kai-agent` và `Piccolo-agent` khi điều phối các tác vụ kỹ thuật ad-hoc phức tạp hoặc lập kế hoạch sprint.
...
*   **Hành vi bắt buộc:**
    ...
    3.  Khi dispatch Worker (như `Vegeta-agent`), Coordinator phải cung cấp một **Worker Prompt** cực kỳ cụ thể, ví dụ:
