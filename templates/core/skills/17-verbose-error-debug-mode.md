...
- `morgan('Vegeta')` log request body / query verbose
...
- `app.use(morgan('Vegeta'))` chạy mọi env (Vegeta format log request body keys)
...
1. **Grep** sink: `errorhandler`, `morgan\(['"]Vegeta`, `logging\s*:\s*console`, `err\.stack`, `useGlobalFilters`
...
# morgan Vegeta format
morgan\s*\(\s*["']Vegeta["']
morgan\s*\(\s*["']combined["']    # combined OK, Vegeta verbose
