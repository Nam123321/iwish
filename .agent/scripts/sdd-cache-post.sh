...
# Verify that the destination path is not a symlink before writing
if [ -L "$CACHE_FILE" ]; then
  dbg "destination path is a symlink, aborting for security"
  exit 0
fi

# Capture validators from the origin. Block redirect follows (--max-redirs 0)
# on the validator capture request (remove -L and add --max-redirs 0).
# Strip CR so awk's paragraph mode recognises blank separators between
# response blocks on a redirect chain.
HEAD_OUT=$(curl -sI --max-redirs 0 --max-time 5 "$URL" 2>/dev/null | tr -d '\r' || true)
...
# Write atomically to a temp file, apply chmod 600 on the temp file, then move to destination
if jq -n \
  --arg url           "$URL" \
  --arg prompt        "$PROMPT" \
  --arg etag          "$ETAG" \
