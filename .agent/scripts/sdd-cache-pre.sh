...
#
# Dependencies: jq, curl, shasum (or sha256sum), python3.
...
# Subnet validation check to block SSRF attempts targeting private/local IP ranges.
if ! python3 -c '
...
try:
    ...
    try:
        ...
        if ip.is_private or not ip.is_global or ip.is_loopback or ip.is_link_local:
            sys.exit(1)
    except ValueError:
        pass
    addr_info = socket.getaddrinfo(hostname, None)
    for info in addr_info:
        clean_ip = info[4][0].split("%")[0]
    ...
    sys.exit(1)
' "$URL"; then
  dbg "URL resolves to a private IP range or is invalid, denying validation"
  exit 0
fi
...
# Reject revalidation if cache file is a symlink to prevent symlink traversal/arbitrary reads
if [ -L "$CACHE_FILE" ]; then
  dbg "cache file is a symlink, rejecting revalidation to prevent symlink traversal"
  exit 0
fi
...
# Perform HEAD request to revalidate cache freshness.
# Disable redirects (--max-redirs 0) to prevent loopback redirections.
