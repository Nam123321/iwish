...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./.agent/scripts/check-bmad-runtime.sh [--mode source|project] [--namespace iwish|legacy-bmad]

Reports I-Wish/BMAD workflow runtime health:
...
while IFS= read -r mapping; do
  ...
  status="$(bmad_runtime_status "$runtime" "$source" "$materialization" "$expected_tmp" "$NAMESPACE")"
