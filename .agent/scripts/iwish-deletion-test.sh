...
if [ -z "$TEST_CMD" ]; then
    ...
    if [ -n "${BMAD_TEST_CMD:-}" ]; then
        TEST_CMD="$BMAD_TEST_CMD"
        echo "📦 Using BMAD_TEST_CMD from environment: $TEST_CMD"
    elif [ -n "${CI:-}" ] || [ ! -t 0 ]; then
        ...
        echo "Set BMAD_TEST_CMD environment variable to specify the test command."
        exit 1
    else
...
TEMP_NAME="${TARGET}.__bmad_deleted"
mv "$TARGET" "$TEMP_NAME"
