...
echo "==========================================================================" >&2
echo "⚠️  WARNING: Materialization via bash is DEPRECATED (IW-STORY-04)." >&2
echo "⚠️  Please use the interactive Universal Agent Pack installer instead:" >&2
echo "⚠️  Run: node scripts/iwish-install.js" >&2
echo "==========================================================================" >&2
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./.agent/scripts/materialize-iwish-runtime.sh [--dry-run|--apply] [--force] [--namespace iwish|legacy-iwish]

Materializes I-Wish or legacy I-Wish workflow runtime files.
...
while IFS= read -r item; do
  ...
  iwish_runtime_generate_expected "$dest" "$source_asset" "$materialization" "$expected_tmp" "$NAMESPACE"
...
if [[ "$APPLY" -eq 1 ]]; then
  ./.agent/scripts/check-iwish-runtime.sh --mode project --namespace "$NAMESPACE"
else
