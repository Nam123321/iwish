...
def print_report(report: ComplianceReport):
    ...
    print(f"""
🛡️ [UIP-DRIFT-DETECTOR] COMPLIANCE REPORT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━""")
    ...
    for item in drift_items:
        findings.append({
            "type": "drift",
            "severity": "high" if item.status == Status.MISSING else "medium",
            "check_id": item.check_id,
            "description": f"{item.spec_definition} → {item.code_implementation}"
        })
        
    result = {
        "source": "uip-drift-detector",
        "status": "success" if report.scs_overall >= 75 else "fail",
        "findings": findings,
        "metrics": {
...
def main():
    parser = argparse.ArgumentParser(description='Spec Compliance Checker (UIP Drift Detector)')
    ...
    parser.add_argument('--log-deviations', action='store_true',
                        help='Log drift findings to _iwish-output/unknowns/deviations.jsonl '
                             '(absorbs uip-deviation-logger functionality)')
    ...
    if args.log_deviations and report.drift_items:
        from datetime import datetime, timezone
        log_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(
            os.path.abspath(__file__)))), '_iwish-output', 'unknowns')
        os.makedirs(log_dir, exist_ok=True)
        log_path = os.path.join(log_dir, 'deviations.jsonl')
