...
## Phase 8b: DUAL-RUN A/B TESTING (🔴 SBRP-Full Only)

> **EVOLUTION LAB EXPERIMENTATION**
> Đối với những bug thuộc Tier 🔴 SBRP-Full (đặc biệt là logic, architecture, security), sau khi có được bản fix từ nội bộ (iwish-native), agent BẮT BUỘC gọi công cụ Evolution Lab để thực hiện kiểm thử A/B.

30b. **Chạy Darwinian Evolver:**
     - Lưu bản code đã fix vào `.agent/evolution-lab/fixtures/[bug-id]/native-fix.js`
     - Chạy `darwinian mutate .agent/evolution-lab/fixtures/[bug-id]/baseline.js` để tạo bản fix từ darwinian
     - So sánh điểm Constraints Retention, Novelty, và Brevity giữa hai phiên bản
     - Chọn ra phiên bản ưu việt nhất trước khi merge

---

## Phase 9: DOCUMENT (Ghi nhận)
...
21. **Cập nhật Hệ miễn dịch (Auto-Immune Update):**
    ...
    - Nếu `hotspot_score >= 30` hoặc `bug_count >= 3`, HOẶC bug là **Type A (Lặp lại)** hoặc **RPN ≥ 60**, BẮT BUỘC:
      ...
      - **Whis Capability Trigger**: Đưa Draft Skill này vào workflow `/enhance-capability` để thực hiện Dual-Run (Inhouse vs Darwinian) đánh giá trước khi promote thành Official Skill.
