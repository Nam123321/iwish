...
- Ensure syntax is valid after edit (run linter for the language)
...
|----------------------|--------|
| 5 | Analyze `loop-results.tsv` for patterns → shift strategy |
...
|--------------|--------------|------------------|
| Multiple changes per iteration | Cannot attribute metric change | One atomic change only |
| Verify before commit | No rollback if verify crashes | Always commit first |
| Editing guard-scope files | Guard becomes meaningless | Guard files are read-only |
| `git reset` instead of `git revert` | Destroys history | Use `git revert` |
| Skipping Phase 1 review | Repeats failed patterns | Always read log + diff |
| Ignoring `Min-Delta` | Micro-improvements cause noise | Set meaningful threshold |
