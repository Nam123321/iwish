...
|-----------|-------------|
| Subjective goals ("make it cleaner") | `/quick-dev-agent` with an approved plan |
| Bug fixing with known root cause | `/fix-bug` workflow |
| One-shot tasks, no repetition needed | `/dev-agent-story` |
...
Scope: src/api/**/*.ts
Verify: npx eslint src/api --format=json 2>/dev/null | node -e "const r=JSON.parse(require('fs').readFileSync('/dev/stdin','utf8')); console.log(r.reduce((a,f)=>a+f.errorCount,0))"
Direction: lower
Iterations: 20
```
...
|----------|--------------------------|
| `/dev-agent-story` | Use after story implementation to polish metrics (coverage, lint) |
| `/qa-agent-automate` | Feed guard commands from QA test suites |
