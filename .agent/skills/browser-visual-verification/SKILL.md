...
- After completing a dev-story implementation (Step 7.5 — between tests and task completion)
- During code-review to verify UI changes claimed in the story
- Whenever a story has UI-related Acceptance Criteria

## Prerequisites

- The application must be running locally (e.g., via `pnpm dev`)
...
If not running:
  1. Run `pnpm dev` in background in the distro/ directory
...
Create a verification section in the story's Dev Agent Record or walkthrough:

```markdown
### 🖥️ Browser Visual Verification

**Date:** {{date}}
**Dev Server Status:** ✅ Running (Admin :3000 | API :3001 | Webstore :3002)
...
- If the dev server fails to start, **HALT** and report the error
...
### In dev-story workflow (Step 7.5)
