...
description: Ensures FE↔BE API route consistency by enforcing use of the shared api-routes.ts contract file. Used during dev-story, code-review, and any API endpoint work.
---
...
## When to Use
- **During `dev-story`**: When implementing any task that adds/modifies API endpoints
...
```typescript
// BAD — relative URL hits Next.js dev server (port 3000), NOT API server (port 3001)
