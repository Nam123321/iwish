...
1.  **Decision Log Entry**: Before executing any command in Caveman Mode, you MUST append a JSONL entry to `_bmad-output/logs/caveman-decisions.log`.
2.  **Schema Consistency**: Every log entry MUST contain `ts`, `intent`, `cmd`, and `files`.
3.  **Audit Trail**: If the user asks for a review of "Caveman actions", use `tail -n 20 _bmad-output/logs/caveman-decisions.log` as the source of truth.
