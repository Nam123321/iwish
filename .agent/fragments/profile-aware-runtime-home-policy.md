...
Use `${IWISH_HOME}` for runtime-only or mirror/export state.

Keep canonical repo assets, repo-local approved memory, and `_iwish/` runtime materialization in their existing project-local locations unless an explicit workflow says otherwise.
...
```text
${IWISH_HOME}/profiles/<profile-id>/projects/<project-slug>/
```
...
Generated drafts begin under `${IWISH_HOME}` and become canonical only after approval and validation.
