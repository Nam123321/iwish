...
13. CRITICAL — UI-UX ORCHESTRATOR FRONTEND REVIEW INVOCATION:
   - If the reviewed change affects layout, styling, spacing, typography, color, charts, responsiveness, interaction behavior, motion, accessibility, or other meaningful user-facing UI behavior:
     ...
     ☐ If specialist guidance is cited in the final review, the reviewer MUST state that final severity remains owned by the BMAD code review workflow, not by the specialist.
     ☐ If specialist guidance conflicts with Design Consultation, UX Guardian, User Simulation Guardian, approved Design System, or approved Stitch visual contract, the stronger BMAD authority wins and the specialist guidance is downgraded to supporting context only.
