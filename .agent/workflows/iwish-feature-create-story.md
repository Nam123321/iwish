...
4.3. **CRITICAL — PRESERVE PLACEHOLDER STORY CONTEXT & VALIDATION PRE-CHECK:**
   - Check if the target story file `@{project-root}/_iwish-output/3. Development/1. Epic & Story/{Feature_Group}/Epic-{epic_id}/Story-{story_id}/story-N.M.md` (or dynamic matching path) already exists.
...
6d. CRITICAL — TIER 1 HYBRID GRAPH UPDATE. Sau khi hoàn thiện và xác thực thành công story file, bạn BẮT BUỘC phải "bơm" trực tiếp tóm tắt story này vào Knowledge Graph bằng lệnh CLI: `iwish inject-node --file "_iwish-output/3. Development/1. Epic & Story/[feature_group]/[epic]/[story].md" --metadata '{"summary": "Mô tả ngắn gọn về tính năng", "tags": ["story", "planning"], "layer": "documentation", "complexity": "low"}'`. Lệnh này giúp FalkorDB nhận diện được node tài liệu này ngay lập tức.
