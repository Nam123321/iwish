...
4.7. CRITICAL — DELETION TEST GATE. Before finalizing the story execution, if the task involved refactoring or adding a new "Shallow Module", you MUST execute the Deletion Test Pre-flight Checklist.
   ...
   - If approved, invoke `bash .agent/scripts/bmad-deletion-test.sh <target-module>` to sandbox the deletion. Ensure the app still compiles and core tests pass after module removal. If they do, the module was shallow and must be rejected.
