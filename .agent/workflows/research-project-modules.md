...
3. **Execution**: If a Library pack is needed, instruct the AI or User to run the global CLI tool:
   ...
   ```
4. **Update Context**: Update `task.md` or context to reflect that new modules (like `ai-engineer-agent` or `data-architect-agent`) are now available in the workspace.
