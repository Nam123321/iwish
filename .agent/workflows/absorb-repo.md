...
3. **Set Runtime Home:** Use `IWISH_HOME=${IWISH_HOME:-~/.iwish-dragonball}`. Runtime artifacts MUST stay under `${IWISH_HOME}` until explicitly promoted.
...
- **Action:** Gap Analysis against existing I-Wish assets.
...
- **Steps:**
  ...
  4. **Compound Check:** Determine if the repo contains >3 independent modules (e.g., a monorepo).
  5. **Track Classification:** Formulate decisions: `ADOPT` (new), `MERGE` (improve existing), `REPLACE` (superior repo pattern), `SKIP` (irrelevant). Đồng thời phân loại đây là `SYSTEM_SKILL` (tích hợp vào I-Wish Core) hay `USER_SPACE` (chỉ dùng cho context của dự án hiện tại).
- **Output:** `${IWISH_HOME}/gap-analysis/{repo-name}-gap-analysis.md` and an index entry in `${IWISH_HOME}/absorbed-repos/{repo-name}/`.
...
### Phase 5.5: ADOPTION REVIEW PACK 🧭 (Agent: Grand-Priest / Whis)
...
- **Steps:**
  1. **Dual-Track Routing:**
     - **If `SYSTEM_SKILL`:** 
       ...
       - Classify module (SKILL_ATTACHMENT, DEDICATED_WORKFLOW, NEW_PERSONA, COMPOUND_INTEGRATION) và tạo Draft Assets.
