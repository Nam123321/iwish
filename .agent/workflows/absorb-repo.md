...
- **Steps:**
  ...
  5. **Track Classification:** Formulate decisions: `ADOPT` (new), `MERGE` (improve existing), `REPLACE` (superior repo pattern), `SKIP` (irrelevant). Đồng thời phân loại đây là `SYSTEM_SKILL` (tích hợp vào BMAD Core) hay `USER_SPACE` (chỉ dùng cho context của dự án hiện tại).
...
- **Steps:**
  1. **Dual-Track Routing:**
     - **If `SYSTEM_SKILL`:** 
       - Tự động gọi luồng **BMAD-SKILL Building** (bắt buộc đi qua PRD -> Epic -> Story).
       - Toàn bộ file PRD, Epic, Story, sprint-status.yaml phải được ép buộc lưu trữ tại thư mục riêng `_bmad-output/bmad-skills/` để tránh ghi đè lên thư mục dự án của user.
