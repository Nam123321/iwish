...
function getHashCachePath(projectRoot) {
    return path.join(projectRoot, '.iwish', 'cache', 'file-hashes.json');
}
function loadHashCache(projectRoot) {
...
async function saveHashCache(projectRoot, cache) {
    const cachePath = getHashCachePath(projectRoot);
...
async function computeSha256Async(filePath) {
...
async function enumerateFilesAsync(projectRoot, ignorePatterns) {
    const files = [];
...
async function scanFiles(projectRoot) {
    ...
    const renamed = [];
    const resolvedAdded = new Set();
    const resolvedDeleted = new Set();
...
function createBatches(files, batchSize = DEFAULT_BATCH_SIZE) {
