...
exports.parseFeatureHierarchy = parseFeatureHierarchy;
exports.parseDesignTokens = parseDesignTokens;
exports.generatePortalHtml = generatePortalHtml;
exports.registerPrototypeCommands = registerPrototypeCommands;
exports.verifyPrototypeSemanticParity = verifyPrototypeSemanticParity;
...
function parseFeatureHierarchy(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (mode === 'overview') {
            ...
            if (trimmed.startsWith('### ')) {
                ...
                const matchedPortal = portals.find(p => headingText.toLowerCase().includes(p.name.toLowerCase()) ||
...
function generatePortalHtml(portal, allPortals, tokens, checksumData) {
...
function findHierarchyPath(projectRoot) {
...
function registerPrototypeCommands(program, getProjectRoot, addSharedDirectoryOption) {
    addSharedDirectoryOption(program
        ...
        if (options.verify) {
            console.log(chalk_1.default.blue('🔍 Verifying prototype checksums...'));
            if (!fs.existsSync(outputDir)) {
                console.error(chalk_1.default.red('❌ Error: Prototypes directory does not exist. Please generate prototypes first.'));
                process.exit(1);
            }
            ...
            if (protoFiles.length === 0) {
                console.error(chalk_1.default.red('❌ Error: No generated prototype files found.'));
                process.exit(1);
            }
            ...
            for (const file of protoFiles) {
                ...
                if (!match) {
                    console.warn(chalk_1.default.yellow(`⚠️ Warning: No metadata block found in ${file}. Marking as out of date.`));
                    verifiedAll = false;
                    continue;
                }
                try {
                    ...
                    if (sourceHierarchy !== activeHierarchyHash || sourceDesign !== activeDesignHash) {
                        console.error(chalk_1.default.red(`❌ Out-of-sync: ${file} (source files have changed since generation)`));
                        verifiedAll = false;
                    }
                    else {
                        console.log(chalk_1.default.green(`    Verified: ${file}`));
                    }
                }
                catch (err) {
                    console.error(chalk_1.default.red(`❌ Error parsing metadata in ${file}: ${err.message}`));
                    verifiedAll = false;
                }
            }
            if (!verifiedAll) {
                console.error(chalk_1.default.red('❌ Verification failed: Some prototypes are out of date or corrupt. Please re-run make-prototype.'));
                process.exit(1);
            }
            console.log(chalk_1.default.green('✅ All prototypes successfully verified!'));
            return;
        }
        console.log(chalk_1.default.blue('Generating interactive HTML prototypes...'));
        if (!fs.existsSync(hierarchyPath)) {
            console.error(chalk_1.default.red(`❌ Error: Feature Hierarchy file not found at ${hierarchyPath}`));
            process.exit(1);
        }
        try {
            ...
            if (portals.length === 0) {
                console.error(chalk_1.default.red('❌ Error: No portals found in feature-hierarchy.md. Please check the overview table.'));
                process.exit(1);
            }
            const checksums = {
            ...
            try {
                ...
                console.error(chalk_1.default.red(`❌ Error creating target directory ${outputDir}: ${err.message}`));
                console.log(chalk_1.default.yellow('Writing fallback prototype skeleton to console...'));
            ...
            const activePortalFiles = new Set();
            ...
            for (const file of existingFiles) {
                if (file.startsWith('prototype-') && file.endsWith('.html')) {
                    if (!activePortalFiles.has(file)) {
                        ...
                        console.log(chalk_1.default.gray(`Purged obsolete file: ${file}`));
                    }
                }
            }
            console.log(chalk_1.default.green('✅ Prototype generation completed successfully!'));
        }
        catch (error) {
            console.error(chalk_1.default.red(`❌ Critical prototype generation error: ${error.message}`));
            process.exit(1);
        }
    }));
    addSharedDirectoryOption(program
        ...
        try {
            ...
            if (result.passed) {
                console.log(chalk_1.default.green('✅ Prototype parity verification PASSED!'));
                console.log(`Verified ${result.portalsCount} portal(s), ${result.screensCount} screen(s), and all design tokens.`);
            }
            else {
                console.error(chalk_1.default.red('❌ Prototype parity verification FAILED!'));
            ...
            console.error(chalk_1.default.red(`❌ Critical verification error: ${error.message}`));
            process.exit(1);
        }
    }));
    addSharedDirectoryOption(program
        ...
        const projectRoot = getProjectRoot(options.directory);
        if (options.sync) {
            console.log(chalk_1.default.blue('🔄 Running Phase 2 SIM Reverse-Sync...'));
            ...
            if (!fs.existsSync(mapPath)) {
                console.error(chalk_1.default.red(`❌ Error: System Integrity Map file not found at ${mapPath}`));
                process.exit(1);
            }
            const epicsAndStoriesPath = path.join(mapDir, '2.4. epics-and-stories.md');
            if (!fs.existsSync(epicsAndStoriesPath)) {
                console.error(chalk_1.default.red(`❌ Error: Epics & Stories file not found at ${epicsAndStoriesPath}`));
                process.exit(1);
            }
            try {
                ...
                for (const epic of epics) {
                    ...
                    if (epic.stories.length === 0) {
                        status = '⚠️ Empty Epic';
                        console.warn(chalk_1.default.yellow(`⚠️ [COVERAGE-GAP] Epic ${epic.id} (${epic.title}) has no stories mapped to any layers.`));
                        hasCoverageGap = true;
                    }
                    else {
                        ...
                        if (hasUI && !hasAPI && !hasDomain) {
                            ...
                            console.warn(chalk_1.default.yellow(`⚠️ [COVERAGE-GAP] Epic ${epic.id} (${epic.title}) contains UI components but lacks corresponding API Endpoints/Domain logic.`));
                            hasCoverageGap = true;
                        }
                        else if ((hasDomain || hasData) && !hasUI && !hasAPI) {
                            status = '⚠️ Orphaned Backend';
                            console.warn(chalk_1.default.yellow(`⚠️ [COVERAGE-GAP] Epic ${epic.id} (${epic.title}) contains Domain/Data engines but lacks corresponding UI components or API Endpoints.`));
                            hasCoverageGap = true;
                        }
                    }
...
`;
                ...
                console.log(chalk_1.default.green(`✓ System Integrity Map successfully reverse-synced at: ${mapPath}`));
                if (hasCoverageGap) {
                    console.warn(chalk_1.default.yellow('⚠️ Reverse-Sync finished with coverage gaps. Please check the warnings above.'));
                }
                else {
                    console.log(chalk_1.default.green('✅ Verification PASSED: All epics are properly aligned across layers!'));
                }
            }
            catch (err) {
                console.error(chalk_1.default.red(`❌ Error running SIM reverse-sync: ${err.message}`));
                process.exit(1);
            }
            return;
        }
        ...
        if (!finalPrdPath) {
            console.error(chalk_1.default.red('❌ Error: PRD file not found. Please run /create-prd or ensure prd.md exists in _iwish-output/2. Product Planning/'));
            process.exit(1);
        }
        ...
        console.log(chalk_1.default.blue('🔍 Analyzing project planning inputs...'));
        console.log(chalk_1.default.gray(`  - Reading PRD: ${path.basename(finalPrdPath)}`));
        console.log(chalk_1.default.gray(`  - Reading Feature Hierarchy: ${finalHierarchyPath ? path.basename(finalHierarchyPath) : 'Not found (using default modules)'}`));
        console.log(chalk_1.default.gray(`  - Reading Cross Dependencies: ${finalDepPath ? path.basename(finalDepPath) : 'Not found (using default bindings)'}`));
        ...
        try {
            ...
            console.warn(chalk_1.default.yellow(`  [Warning] Failed to parse PRD file: ${e.message}`));
        }
        ...
        if (finalHierarchyPath) {
            try {
                ...
                console.warn(chalk_1.default.yellow(`  [Warning] Failed to parse Feature Hierarchy: ${e.message}`));
            }
        }
        ...
        if (finalDepPath) {
            try {
                ...
                console.warn(chalk_1.default.yellow(`  [Warning] Failed to parse Cross Dependencies: ${e.message}`));
            }
        }
        ...
        console.log(chalk_1.default.green(`✓ Successfully extracted metadata for: "${projectName}"`));
        console.log(chalk_1.default.gray(`  Features found: ${features.length} (evaluating top ${displayFeatures.length})`));
        console.log(chalk_1.default.gray(`  Dependencies detected: ${dependencies.length} connections`));
        console.log('');
        console.log(chalk_1.default.blue('Generating 5 Architectural Topology Options tailored to project context:'));
        console.log('');
        const simOptions = [
            {
                id: 1,
  ...
  - Schemas: Database schema tables corresponding to ${displayFeatures.join(', ')}.`
        ...
        simOptions.forEach(opt => {
            console.log(chalk_1.default.yellow(`Option ${opt.id}: ${opt.name}`));
        ...
        if (options.select) {
            ...
            if (!chosenOpt) {
                console.error(chalk_1.default.red(`❌ Invalid option selection: ${options.select}. Choose a value from 1 to 5.`));
                process.exit(1);
            }
            console.log(chalk_1.default.green(`✓ Selected Option ${selectedId}: ${chosenOpt.name}`));
            const mapDir = path.join(projectRoot, '_iwish-output', '2. Product Planning');
...
`;
            ...
            console.log(chalk_1.default.green(`✓ System Integrity Map saved successfully to: ${mapPath}`));
        }
        else {
            console.log(chalk_1.default.cyan('Run this command with `--select <1-5>` to select and write your final SIM map.'));
        }
    }));
}
async function verifyPrototypeSemanticParity(projectRoot) {
    const hierarchyPath = findHierarchyPath(projectRoot);
    const designPath = findDesignPath(projectRoot);
    const outputDir = path.join(projectRoot, '_iwish-output', '2. Product Planning', 'prototypes');
    const errors = [];
    ...
    const whitelistedColors = new Set([
        '#ffffff', '#000000',
        '#fff', '#000',
    ...
    if (tokens.colors.primaryGreen)
        whitelistedColors.add(normalizeHex(tokens.colors.primaryGreen));
    if (tokens.colors.secondaryGreen)
        whitelistedColors.add(normalizeHex(tokens.colors.secondaryGreen));
    if (tokens.colors.backgroundDark)
        whitelistedColors.add(normalizeHex(tokens.colors.backgroundDark));
    if (tokens.colors.textLight)
        whitelistedColors.add(normalizeHex(tokens.colors.textLight));
    if (tokens.colors.grayBorder)
        whitelistedColors.add(normalizeHex(tokens.colors.grayBorder));
    for (const portal of portals) {
        const fileName = `prototype-${portal.slug}.html`;
        const filePath = path.join(outputDir, fileName);
        if (!fs.existsSync(filePath)) {
        ...
        let dom;
        try {
            dom = new jsdom_1.JSDOM(fileContent, { runScripts: "outside-only" });
        }
        catch (err) {
