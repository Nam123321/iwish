...
exports.scaffoldBrandQuestionnaire = scaffoldBrandQuestionnaire;
exports.validateBrandQuestionnaire = validateBrandQuestionnaire;
exports.checkDesignConnection = checkDesignConnection;
exports.installDesignTool = installDesignTool;
exports.generateLogoAssets = generateLogoAssets;
exports.generateLogoConcepts = generateLogoConcepts;
exports.generatePlatformPrompts = generatePlatformPrompts;
exports.validateLogoLock = validateLogoLock;
exports.auditAndRefactorLogo = auditAndRefactorLogo;
exports.generateBrandGuidelines = generateBrandGuidelines;
const fs = __importStar(require("fs-extra"));
const path = __importStar(require("path"));
const runtime_1 = require("./runtime");
// Story 12.1: Strategy Intake & Rebranding Audit Questionnaire
async function scaffoldBrandQuestionnaire(projectRoot) {
...
async function validateBrandQuestionnaire(projectRoot) {
    ...
    if (isRefactoring) {
        if (content.match(/\[x\]\s+\*\*Path A/i) || content.match(/\[x\]\s+Path A/i))
            refactoringPath = 'A';
        else if (content.match(/\[x\]\s+\*\*Path B/i) || content.match(/\[x\]\s+Path B/i))
            refactoringPath = 'B';
        else if (content.match(/\[x\]\s+\*\*Path C/i) || content.match(/\[x\]\s+Path C/i))
            refactoringPath = 'C';
        else {
    ...
    if (content.match(/\[x\]\s+\*\*stitch\*\*/i) || content.match(/\[x\]\s+stitch/i))
        designTool = 'stitch';
    else if (content.match(/\[x\]\s+\*\*figma\*\*/i) || content.match(/\[x\]\s+figma/i))
        designTool = 'figma';
    else if (content.match(/\[x\]\s+\*\*claude-design\*\*/i) || content.match(/\[x\]\s+claude-design/i))
        designTool = 'claude-design';
    else if (content.match(/\[x\]\s+\*\*canva\*\*/i) || content.match(/\[x\]\s+canva/i))
        designTool = 'canva';
    else if (content.match(/\[x\]\s+\*\*Local File Exporter\*\*/i) || content.match(/\[x\]\s+Local File Exporter/i))
        designTool = 'local';
    else {
...
async function checkDesignConnection(projectRoot) {
    const profile = (0, runtime_1.readToolProfile)(projectRoot);
    if (!profile || !profile.selections || !profile.selections.design) {
        return null;
    }
    return profile.selections.design;
}
async function installDesignTool(projectRoot, selection) {
    if (selection === 'local') {
        // Story 12.4 AC3: Local File Exporter fallback option
        await (0, runtime_1.selectToolProfile)(projectRoot, 'design', 'local');
    ...
    // Update profile
    await (0, runtime_1.selectToolProfile)(projectRoot, 'design', selection);
...
async function generateLogoAssets(projectRoot, logoSelection) {
...
function getLogoSVGs(logoSelection, brandName) {
...
async function generateLogoConcepts(projectRoot, optionsCount) {
...
function generatePlatformPrompts(concept, customToolGuideline) {
...
async function validateLogoLock(projectRoot, bypassOverride = false) {
...
async function auditAndRefactorLogo(projectRoot, pathSelection, existingAssetPath) {
...
async function generateBrandGuidelines(projectRoot, logoSelection) {
    const targetDir = path.join(projectRoot, '_iwish-output', 'brand-identity');
    await fs.ensureDir(targetDir);
...
`;
    ...
    const htmlGuidelineContent = getHTMLGuidelineTemplate(brandName, culture, personality, physique, relationship, reflection, selfImage, logoName, logoDesc, typography, typographyRationale, svgs);
    await fs.writeFile(path.join(targetDir, 'brand-guideline.html'), htmlGuidelineContent, 'utf8');
    return guidelinePath;
}
function getHTMLGuidelineTemplate(brandName, culture, personality, physique, relationship, reflection, selfImage, logoName, logoDesc, typography, typographyRationale, svgs) {
