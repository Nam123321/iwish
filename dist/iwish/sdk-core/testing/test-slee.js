...
const file_system_store_1 = require("../slee/file-system-store");
const slee_orchestrator_1 = require("../slee/slee-orchestrator");
async function runTests() {
    console.log('=== Starting SLEE Integration Tests ===');
    const dispatcher = new dummy_modules_1.DummyEventDispatcher();
    const store = new file_system_store_1.FileSystemSleeStore('./_iwish-output/slee-data-test'); // use test dir
    // Clean up previous test runs
    const fs = require('fs/promises');
    await fs.rm('./_iwish-output/slee-data-test', { recursive: true, force: true });
    await store.init();
    const orchestrator = new slee_orchestrator_1.SleeOrchestrator(dispatcher, store);
    // Dispatch an error
