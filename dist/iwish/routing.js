...
exports.resolveLegacyName = resolveLegacyName;
...
function detectCommand(normalizedRequest) {
    if (normalizedRequest.startsWith('/')) {
        const exact = normalizedRequest.split(/\s+/)[0];
        const canonical = constants_1.LEGACY_COMMAND_ALIASES[exact] || exact;
        return {
    ...
    if (normalizedRequest.includes('github') &&
    ...
    if (/\b(research|search|find|compare|look for|discover)\b.*\b(github|repo|repository|framework|package|module|open source|open-source|solution|skill)\b/.test(normalizedRequest) ||
...
function getGraphStatus(projectRoot) {
    const runtimeGraphProfile = path.join((0, constants_1.getRuntimeRoot)(projectRoot, 'iwish'), 'graphs', 'graph-profile.yaml');
    ...
    const graphProfile = fs.existsSync(runtimeGraphProfile)
        ? yaml_1.default.parse(fs.readFileSync(runtimeGraphProfile, 'utf8'))
        : null;
    if (fs.existsSync(runtimeGraphProfile) &&
...
function getKeywordScore(normalizedRequest, canonicalCommand) {
    if (canonicalCommand === '/research-solution-sources') {
        if (/\b(github|repo|repository|skill|framework|package|module|solution|compare|research)\b/.test(normalizedRequest))
            return 18;
    }
    if (canonicalCommand === '/make-story') {
        if (/\b(story|epic|acceptance criteria|backlog)\b/.test(normalizedRequest))
            return 18;
    }
    if (canonicalCommand === '/code') {
        if (/\b(implement|code|fix|patch|feature)\b/.test(normalizedRequest))
            return 18;
    }
...
function isImplementationIntent(normalizedRequest) {
    return /\b(fix|implement|code|patch|bug|ship|complete|build)\b/.test(normalizedRequest);
}
function isPlanningIntent(normalizedRequest) {
    return /\b(create|draft|write|refine|story|epic|acceptance criteria|spec|plan)\b/.test(normalizedRequest);
}
function computeScoring(projectRoot, normalizedRequest, canonicalCommand, routeReason, truthMatches, truth, routeProfileExists) {
    const evidence = [];
    ...
    const recentStoryOverlap = recentDecisions.some((decision) => (decision.contextScope?.matchedStories || []).some((storyId) => truthMatches.stories.includes(storyId)));
    ...
    const confidenceBand = confidencePercent >= 90 ? 'direct' : confidencePercent >= 70 ? 'options' : 'clarify';
    return {
...
function buildRecommendations(canonicalCommand, normalizedRequest) {
...
async function routeRequest(projectRoot, request) {
    const normalizedRequest = request.trim().toLowerCase();
    const status = (0, runtime_1.getStatus)(projectRoot);
    const sourceOfTruth = (0, source_of_truth_1.loadSourceOfTruth)(projectRoot);
    const truthMatches = (0, source_of_truth_1.findSourceOfTruthMatches)(projectRoot, request);
    ...
    const activeWorkflowPath = path.join((0, constants_1.getRuntimeRoot)(projectRoot, 'iwish'), 'runtime', 'workflows', 'active-workflow.json');
    ...
    const storyLooksReviewPending = Boolean(strongestMatchedStory &&
        strongestMatchedStory.readiness === 'high' &&
        ['review', 'ready-for-review', 'ready_for_review'].includes(strongestStoryState));
    const storyLooksCompleted = Boolean(strongestMatchedStory &&
        strongestMatchedStory.readiness === 'high' &&
        ['completed', 'done', 'closed'].includes(strongestStoryState));
    const storyLooksExecutionReady = Boolean(strongestMatchedStory &&
        strongestMatchedStory.readiness === 'high' &&
        ['ready', 'in-progress', 'in_progress'].includes(strongestStoryState));
    const storyLooksPlanningFirst = Boolean(strongestMatchedStory &&
        (strongestMatchedStory.readiness !== 'high' ||
            ['todo', 'draft', 'backlog', 'planning', 'unknown', ''].includes(strongestStoryState)));
    const route = (truthMatches.stories.length > 0 || truthMatches.epics.length > 0) &&
        ...
        ? {
            canonicalCommand: storyLooksReviewPending
                ? '/review'
            ...
            targetAgent: storyLooksReviewPending
                ? 'review-agent'
                : storyLooksCompleted && !implementationIntent && !planningIntent
                    ? 'orch-agent'
                    : storyLooksCompleted && implementationIntent
            ...
            routeReason: storyLooksReviewPending && strongestMatchedStory
        ...
        : initialRoute;
    const candidateCatalogEntries = (0, catalog_1.searchCatalog)(projectRoot, normalizedRequest).map((entry) => ({
        id: entry.id,
        type: entry.type,
    ...
    const recommendedQueueType = route.canonicalCommand === '/absorb-repo'
        ? 'repo-absorption'
        : route.canonicalCommand === '/make-ui-spec'
            ? 'design-tweak'
            : route.canonicalCommand === '/enhance-skill'
    ...
    const outputDir = path.join((0, constants_1.getRuntimeRoot)(projectRoot, 'iwish'), 'runtime', 'route-decisions');
...
function resolveLegacyName(name) {
    const normalized = name.trim().toLowerCase();
    return constants_1.LEGACY_AGENT_ALIASES[normalized] || constants_1.LEGACY_COMMAND_ALIASES[normalized] || normalized;
}
