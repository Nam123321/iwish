...
exports.collectEpicInterfaces = collectEpicInterfaces;
exports.generateMockObjects = generateMockObjects;
exports.writeLockManifest = writeLockManifest;
exports.checkWriteLock = checkWriteLock;
exports.verifyLockIntegrity = verifyLockIntegrity;
exports.activateInterfaceLockGate = activateInterfaceLockGate;
 ...
 */
function collectEpicInterfaces(epicId, dagJson, projectRoot) {
    const collected = [];
    ...
    if (fs.existsSync(libDir))
        scanDirs.push(libDir);
    if (fs.existsSync(appDir))
        scanDirs.push(appDir);
 ...
 */
function findFilesRecursive(dir, projectRoot) {
    const results = [];
 ...
 */
function extractTypeScriptInterfaces(content, filePath, storyIds) {
    const interfaces = [];
 ...
 */
function extractTypeAliases(content, filePath, storyIds) {
    const types = [];
 ...
 */
function extractZodSchemas(content, filePath, storyIds) {
    const schemas = [];
 ...
 */
function extractComponentProps(content, filePath, storyIds) {
    const props = [];
 ...
 */
function inferTypesFromJSDoc(content, filePath, storyIds) {
    const inferred = [];
 ...
 */
function extractMigrationSchemas(content, filePath, storyIds) {
    const schemas = [];
 ...
 */
function parseZodProperties(body) {
    const properties = [];
    const lines = body.split('\n');
    for (const line of lines) {
        const trimmed = line.trim();
 ...
 */
function zodTypeToTsType(zodType) {
    const map = {
        string: 'string',
        number: 'number',
        boolean: 'boolean',
 ...
 */
function generateMockObjects(interfaces) {
    const mocks = [];
    for (const iface of interfaces) {
        // Skip migration schemas from mock generation if they have no useful properties
        if (iface.kind === 'db-migration' && iface.properties.length === 0)
            continue;
 ...
 */
function generateFactoryFunction(iface) {
 ...
 */
function generateExampleValue(iface) {
    const obj = {};
...
function getMockValueForType(type, name) {
    ...
    if (nameLower.includes('name'))
        return `'Mock ${name}'`;
    if (nameLower.includes('date') || nameLower.includes('at') || nameLower.includes('timestamp'))
        return `new Date().toISOString()`;
    if (nameLower.includes('count') || nameLower.includes('total') || nameLower.includes('amount'))
        return `0`;
    if (nameLower.includes('enabled') || nameLower.includes('active') || nameLower.includes('is'))
        return `true`;
    ...
    if (cleanType.startsWith('Set<'))
        return `new Set()`;
    if (cleanType === 'unknown' || cleanType === 'any')
        return `null`;
    return `{} as any`;
}
/**
 ...
 */
function getExampleValueForType(type, name) {
    const nameLower = name.toLowerCase();
 ...
 */
function writeLockManifest(epicId, interfaces, mocks, projectRoot, outputBaseDir) {
 ...
 */
function generateContractsFile(interfaces, epicId) {
 ...
 */
function generateMocksFile(mocks, epicId) {
    const header = `/**
 ...
 */
function generateMigrationsFile(migrations, epicId) {
 ...
 */
function checkWriteLock(targetFilePath, epicId, projectRoot) {
    const manifestPath = path.join(projectRoot, '_iwish-output', `${LOCK_DIR_PREFIX}${epicId}`, MANIFEST_FILE);
 ...
 */
function verifyLockIntegrity(epicId, projectRoot) {
    const manifestPath = path.join(projectRoot, '_iwish-output', `${LOCK_DIR_PREFIX}${epicId}`, MANIFEST_FILE);
    if (!fs.existsSync(manifestPath)) {
    ...
    const tamperedFiles = [];
    for (const entry of manifest.files) {
        if (entry.artifactType === 'manifest')
 ...
 */
async function activateInterfaceLockGate(epicId, options = {}) {
    ...
    const lockDir = path.join(options.outputBaseDir || path.join(projectRoot, '_iwish-output'), `${LOCK_DIR_PREFIX}${epicId}`);
    if (fs.existsSync(lockDir) && !options.force) {
        ...
        const existingManifest = fs.readJsonSync(path.join(lockDir, MANIFEST_FILE));
        return {
            epicId,
            success: true,
            outputDir: lockDir,
    ...
    const manifest = writeLockManifest(epicId, interfaces, mocks, projectRoot, options.outputBaseDir);
    const outputDir = path.join(options.outputBaseDir || path.join(projectRoot, '_iwish-output'), `${LOCK_DIR_PREFIX}${epicId}`);
