...
exports.SwarmStateBoardManager = void 0;
exports.buildSubagentPrompt = buildSubagentPrompt;
exports.validateWritePermission = validateWritePermission;
exports.onStoryComplete = onStoryComplete;
exports.spawnWave = spawnWave;
exports.executeSwarmOrchestration = executeSwarmOrchestration;
 ...
 */
class SwarmStateBoardManager {
    state;
    outputPath;
    constructor(epicId, dag, platform, projectRoot, outputDir) {
     ...
     */
    markRunning(storyId, conversationId, branch) {
     ...
     */
    markCompleted(storyId, status, error) {
        const entry = this.state.stories.find((s) => s.storyId === storyId);
        if (entry) {
            entry.status = status;
     ...
     */
    isComplete() {
        return this.state.stories.every((s) => ['PASS', 'FAIL', 'BLOCKED', 'TIMEOUT', 'INTEGRATION-PASSED'].includes(s.status));
    }
    /**
     * Check if all stories passed.
     */
    isAllPassed() {
        return this.state.stories.every((s) => s.status === 'PASS' || s.status === 'INTEGRATION-PASSED');
    }
    computeSummary(stories) {
        return {
 ...
 */
async function buildSubagentPrompt(storyId, epicId, projectRoot) {
...
`;
    ...
    let contextSection = '';
    try {
        const storyUri = (0, okf_helper_1.formatOKFUri)(storyPath, projectRoot);
        const { getCompressedContextURIs } = await Promise.resolve().then(() => __importStar(require('./graph-db')));
 ...
 */
function resolveAgentName(storyId) {
 ...
 */
function validateWritePermission(agentStoryId, filePath, epicId, projectRoot) {
    // Import checkWriteLock from interface-lock dynamically to avoid circular deps
    const lockManifestPath = path.join(projectRoot, '_iwish-output', `interface-lock-epic-${epicId}`, 'lock-manifest.json');
 ...
 */
function onStoryComplete(storyId, result, stateBoard, dag) {
    const newlyReady = [];
    if (result.status === 'PASS') {
        ...
        for (const depId of dependents) {
            ...
            if (depEntry && depEntry.status === 'PENDING') {
                stateBoard.markBlocked(depId, `Blocked: dependency ${storyId} ${result.status.toLowerCase()}.`);
                ...
                for (const transId of transitiveDeps) {
                    ...
                    if (transEntry && transEntry.status === 'PENDING') {
                        stateBoard.markBlocked(transId, `Transitively blocked: upstream dependency ${storyId} ${result.status.toLowerCase()}.`);
                    }
                }
            }
        }
    ...
    const waveComplete = waveStories.every((s) => ['PASS', 'FAIL', 'BLOCKED', 'TIMEOUT', 'INTEGRATION-PASSED'].includes(s.status));
    if (waveComplete) {
 ...
 */
function findDependentStories(storyId, dag) {
    const dependents = [];
 ...
 */
async function spawnWave(waveStories, epicId, dag, stateBoard, api, projectRoot) {
    const results = [];
    const waveNum = stateBoard.getState().currentWave;
    // Build spawn configs for each story
    const configs = await Promise.all(waveStories.map(async (storyId) => {
...
// ---------------------------------------------------------------------------
function updateAgentTrace(projectRoot, id, label, duration, tokens, parentId = null) {
    ...
    const existingIndex = trace.findIndex((t) => t.id === id);
    const entry = {
        id,
        label,
        group: 'agent',
 ...
 */
async function executeSwarmOrchestration(epicId, api, options = {}) {
    ...
    if (!fs.existsSync(dagPath)) {
        throw new Error(`No dependency map found at ${dagPath}. ` +
    ...
    const platform = (options.forceSequential || !api.supportsParallel())
        ? 'SEQUENTIAL_FALLBACK'
        : 'PARALLEL';
    ...
    const stateBoard = new SwarmStateBoardManager(epicId, dag, platform, projectRoot, options.outputDir);
    ...
    for (let waveIdx = 0; waveIdx < dag.execution_waves.length; waveIdx++) {
        ...
        // Spawn wave
        const results = await spawnWave(pendingStories, epicId, dag, stateBoard, api, projectRoot);
        // Process results
 ...
 */
function printExecutionPlan(dag, stateBoard) {
    console.log(`\n📋 Execution Plan:`);
