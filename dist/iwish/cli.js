...
async function promptGraphToolSelection(projectRoot) {
    ...
    try {
        console.log('');
        console.log(chalk_1.default.yellow('Graph setup is required for Orch brain surfaces.'));
        console.log(chalk_1.default.blue('Choose a graph solution now, or press Enter to configure it later.'));
        ...
        if (!answer) {
            console.log(chalk_1.default.yellow('Skipped graph setup for now.'));
            console.log(`Run later: iwish select-tool graph ${graphPrompt.recommended || '<adapter>'}`);
            return;
        }
        const numeric = Number(answer);
        const selected = Number.isInteger(numeric) && numeric >= 1 && numeric <= graphPrompt.options.length
...
async function runCli() {
    ...
    addSharedDirectoryOption(program
        ...
        await (0, runtime_1.registerModule)(getProjectRoot(options.directory), source, options.name, {
    ...
    addSharedDirectoryOption(program
        ...
        .action(async (options) => {
        const result = await (0, runtime_1.generateCapabilityReviewPack)(getProjectRoot(options.directory), {
            name: options.name,
        ...
        console.log(chalk_1.default.green(`Generated review pack for ${options.name}`));
    ...
    addSharedDirectoryOption(program
        ...
        const decision = await (0, routing_1.routeRequest)(getProjectRoot(options.directory), request);
    ...
    addSharedDirectoryOption(program
        ...
        .option('--format <type>', 'json or markdown', 'json')
        .action((options) => {
        const report = (0, skill_graph_1.buildSkillGraphReport)(getProjectRoot(options.directory));
        if (options.format === 'markdown') {
    ...
    addSharedDirectoryOption(program
        ...
        .option('-n, --name <name>', 'Display name for the registered module')
        .action(async (moduleSource, options) => {
        console.log(chalk_1.default.yellow('`add` is a compatibility alias. Use `iwish register-module` moving forward.'));
        await (0, runtime_1.registerModule)(getProjectRoot(options.directory), moduleSource, options.name);
    }));
