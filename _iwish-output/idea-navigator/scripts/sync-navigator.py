...
# Paths are relative to this script: _iwish-output/idea-navigator/scripts/
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
I-Wish_OUTPUT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, '..', '..'))
NAV_JS_FILE = os.path.join(I-Wish_OUTPUT_DIR, 'idea-navigator', 'js', 'navigator-data.js')

MIN_CONFIDENCE = 60
AMBIGUITY_DELTA = 10
VALID_PHASES = {'origin', 'spark', 'deep_dive', 'forge'}
...
def guess_phase_from_path(filepath):
    ...
    if '/planning/idea-challenges/' in path_lower:
        return 'spark'
    ...
    if '/stories/' in path_lower:
        return 'forge'
    if base in ['prd.md', 'product-brief.md', 'architecture.md', 'project-context.md']:
        return 'origin'
    return None

def normalize_phase_value(raw_phase, filepath):
    if not raw_phase:
        return None
...
def process_file(filepath, candidate_filenames):
    ...
    if not match:
        return None # Not a iwish markdown file with yaml frontmatter

    yaml_text = match.group(1)
    body_text = match.group(2)
    yaml_data = parse_yaml_lines(yaml_text)

    original_phase = yaml_data.get('phase')
    normalized_phase = normalize_phase_value(original_phase, filepath)
    if not normalized_phase:
        print(f"⚠️ Missing or unmappable phase for {os.path.basename(filepath)}")
        return None

    if original_phase != normalized_phase:
        yaml_lines = yaml_text.strip().split('\n')
        phase_updated = False
        for idx, line in enumerate(yaml_lines):
            if line.strip().startswith('phase:'):
                yaml_lines[idx] = f'phase: "{normalized_phase}"'
                phase_updated = True
                break
        if not phase_updated:
            yaml_lines.append(f'phase: "{normalized_phase}"')
        new_yaml_text = '\n'.join(yaml_lines)
        new_content = f'---\n{new_yaml_text}\n---\n{body_text}'
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(new_content)
        heal_reason = 'missing' if not original_phase else f'invalid ({original_phase})'
        print(f"✅ Auto-healed {heal_reason} phase for {os.path.basename(filepath)} -> {normalized_phase}")
        yaml_data['phase'] = normalized_phase
    ...
    # Extract Admiralty Score
    admiralty_score = yaml_data.get('admiralty') or yaml_data.get('reliability')
    if not admiralty_score:
        # Search in body text
        match = re.search(r'\[(?:Reliability|Confidence|Admiralty):\s*([A-F][1-6])\]', body_text, re.I)
        if match:
            admiralty_score = match.group(1).upper()
    
    # Extract Logic Steps / Timeline
    timeline = []
    # Look for lists or lines that look like sequential steps
    step_patterns = [
        r'^(?:Step\s*\d+[:.]?|\d+[:.)])\s+(.*)',
        r'^\-\s+Step\s+\d+[:.]?\s+(.*)'
    ]
    for block in blocks:
        ...
        for line in lines:
            ...
            for pattern in step_patterns:
                step_match = re.match(pattern, line, re.I)
                if step_match:
    ...
    return {
        "filepath": os.path.relpath(filepath, I-Wish_OUTPUT_DIR),
        "phase": phase,
        "relations": relations,
        "data": {
            "id": id_val,
            ...
            "hasPivot": has_pivot,
            "pivots": pivots,
            "admiraltyScore": admiralty_score,
            "timeline": timeline
        }
    }

def main():
    print("🚀 Starting I-Wish Navigator Sync Engine...")
    ...
    search_pattern = os.path.join(I-Wish_OUTPUT_DIR, '**', '*.md')
    ...
    candidate_filenames = [os.path.relpath(f, I-Wish_OUTPUT_DIR) for f in md_files]
    ...
    js_content = f"window.I-Wish_NAV_DATA = {json_payload};\n"
    
    with open(NAV_JS_FILE, 'w', encoding='utf-8') as f:
        f.write(js_content)
        
    print(f"✅ Sync complete! Processed {processed_count} files into {os.path.relpath(NAV_JS_FILE, I-Wish_OUTPUT_DIR)}")
