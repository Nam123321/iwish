...
# ==============================================================
echo -e "\n${YELLOW}[1/6] Updating iwish-feature-code-review.md...${NC}"

# Check if already updated
if grep -q "ADR-002" "$LDM/.agent/workflows/iwish-feature-code-review.md" 2>/dev/null; then
  echo -e "${GREEN}  → Already up-to-date, skipping${NC}"
else
  # Remove the old </steps> closing tag first, then add new steps + closing tag
  sed -i '' 's|</steps>||g' "$LDM/.agent/workflows/iwish-feature-code-review.md"

  cat >> "$LDM/.agent/workflows/iwish-feature-code-review.md" << 'EOF'
...
grep -c "ADR-002" "$LDM/.agent/workflows/iwish-feature-code-review.md" && echo " refs" || echo "❌"
